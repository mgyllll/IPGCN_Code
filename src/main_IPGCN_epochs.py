...
if __name__ == '__main__':
    ...
    for q in range(20, 55, 5):
        ...
        for ep in range(5, 205, 5):
            ...
            PCNNList.append([ep, pcnnList['accuracy'].mean(), pcnnList['precision'].mean(), pcnnList['recall'].mean(),
                             pcnnList['f1'].mean(),
                             pcnnList['auc'].mean()])
        PCNNList = pd.DataFrame(PCNNList, columns=['ep', 'accuracy', 'precision', 'recall', 'f1', 'auc'])
        PCNNList.to_csv(data_path + '/rs20231224_epochs_' + str(q) + '.csv', index=False, encoding="utf_8_sig")
