    ...
    for q in range(5, 55, 5):
        ...
        for L in range(4, 104, 4):
            ...
            for train_idx, val_idx in kfold.split(sampleX, labelY):
                ...
                rcnn = Models.CNN1(L)
                rcnn, loss = Utils.train_model1(train_loader, rcnn, num_epochs, lr, path=None)
            ...
            PCNNList.append([L, pcnnList['accuracy'].mean(), pcnnList['precision'].mean(), pcnnList['recall'].mean(),
                             pcnnList['f1'].mean(),
                             pcnnList['auc'].mean()])
        PCNNList = pd.DataFrame(PCNNList, columns=['L', 'accuracy', 'precision', 'recall', 'f1', 'auc'])
        PCNNList.to_csv(exp_file + '/rs20231224_L_' + str(q) + '.csv', index=False, encoding="utf_8_sig")
