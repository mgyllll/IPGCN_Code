...
def batch_process(df, path, batch, ii):
    global object_dict
    print('objects_network_u50_' + str(ii) + '.csv')
    common_review_u = list()
    for i, j in tqdm(batch):
        if object_dict.get(i) is not None:
            u1 = object_dict[i]
        else:
            u1 = set(df.loc[df['movie_id'] == i, 'user_id'])
            object_dict[i] = u1
        if object_dict.get(j) is not None:
            u2 = object_dict[j]
        else:
            u2 = set(df.loc[df['movie_id'] == j, 'user_id'])
            object_dict[j] = u2
        uu = u1 & u2
        if uu:
            common_review_u.append([i, j, len(u1), len(u2), len(uu), len(uu) / (len(u1) + len(u2) - len(uu))])
    users_net = pd.DataFrame(common_review_u, columns=['i', 'j', 'oi', 'oj', 'oij', 'relation'])
    users_net.to_csv(path + '/objects_network_u50_' + str(ii) + '.csv', index=False, encoding="utf_8_sig")
...
def first_stop(df1, df2):
    ...
    for q in tqdm(range(100, -1, -1)):
        ...
        p = len(set(df['i']) | set(df['j'])) / len(set(df1['movie_id']))
        r = 1 - len(df) / len(df2)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(path + 'objects_network_u50_filter.csv'):
        # 二分网络映射为对象单部网络，按批次处理：30000000
        process_combinations(ratings_u50, path, 30000000)

        # 合并映射网络
        no = 0
        fpath1 = path + 'objects_network_u50_' + str(no) + '.csv'
        ...
        comm.to_csv(path + '/objects_network_u50.csv', index=False, encoding="utf_8_sig")
        # 根据边的绝对强度进行剪枝
        comm_ml_100k_part, pruning_strategy = first_stop(ratings_u50, comm)
        pruning_strategy.to_csv(path + '/objects_pruning_strategy.csv', index=False, encoding="utf_8_sig")
        comm_ml_100k_part.to_csv(path + '/objects_network_u50_filter.csv', index=False, encoding="utf_8_sig")
